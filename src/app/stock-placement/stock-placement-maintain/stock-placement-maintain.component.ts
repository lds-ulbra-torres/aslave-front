...
export class StockPlacementMaintainComponent implements OnInit {
  ...
  onSubmit(form): boolean{
    ...
    console.log(JSON.stringify(theHell))
    this._stockPlacementService.postStockInput2(theHell).subscribe(
      data => JSON.stringify(this.stockProducts), 
      error => console.log(error),
      () => console.log("nao entendi nada")
    )
